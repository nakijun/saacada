...
#else
  dlna = (dlna_t *)malloc (sizeof (dlna_t));
#endif
  dlna->inited = 1;
  dlna->verbosity = 0;
...
#else
  codecs = (av_codecs_t *)malloc (sizeof (av_codecs_t));
#endif
...
dlna_guess_media_profile (dlna_t *dlna, const char *filename)
{
  ...
  p = dlna->first_profile;
#else
  p = (dlna_registered_profile_t *)dlna->first_profile;
#endif
  while (p)
  {
    dlna_profile_t *prof;
