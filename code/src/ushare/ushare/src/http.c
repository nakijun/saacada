...
#else
  fd = open (entry->fullpath, _O_RDONLY | _O_BINARY);
#endif
  if (fd < 0)
    return NULL;
#ifdef WIN32
...
#ifdef WIN32
    len = (size_t) MIN (buflen, unsigned(file->detail.memory.len - file->pos));
#else
...
struct UpnpVirtualDirCallbacks* get_virtual_dir_callbacks()
{
