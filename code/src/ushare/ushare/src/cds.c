...
didl_add_item (struct buffer_t *out, int item_id,
               int parent_id, char *restricted, char *class_, char *title,
               char *protocol_info, off_t size, char *url, char *filter)
{
  ...
  didl_add_tag (out, DIDL_ITEM_CLASS, class_);
  ...
  if (filter_has_val (filter, DIDL_RES))
  {
    ...
    if (url)
    {
	  ...
	  buffer_appendf (out, "http://%s:%d/%s",
		  UpnpGetServerIpAddress (), ut->m_pRLServer->GetHttpPort(), url);
#endif
	}
    buffer_appendf (out, "</%s>", DIDL_RES);
  }
                    ...
                    char *title, char *class_)
{
  ...
  didl_add_tag (out, DIDL_CONTAINER_CLASS, class_);
...
#ifdef NO_USHARE_MAIN
	  ut->m_pRLServer->GetChannelMgr().ExportMedia(entry->url, entry->fullpath);
#endif
...
#ifdef NO_USHARE_MAIN
	  ut->m_pRLServer->GetChannelMgr().ExportMedia((*childs)->url, (*childs)->fullpath);
#endif
