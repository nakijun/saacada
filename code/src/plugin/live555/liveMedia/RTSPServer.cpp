...
::RTSPClientSession(RTSPServer& ourServer, unsigned sessionId,
	      int clientSocket, struct sockaddr_in clientAddr, bool isAttached, IRTSPClientSessionObserver* observer)
  : fOurServer(ourServer), fIsAttached(isAttached), fObserver(observer), fOurSessionId(sessionId),
    fOurServerMediaSession(NULL),
    fClientSocket(clientSocket), fClientAddr(clientAddr),
    fHandleRequestTask(NULL), fCloseClientSessionTask(NULL), fLivenessCheckTask(NULL),
...
RTSPServer::RTSPClientSession::~RTSPClientSession() {
  ...
  envir().taskScheduler().unscheduleDelayedTask(fHandleRequestTask);
  envir().taskScheduler().unscheduleDelayedTask(fCloseClientSessionTask);
...
bool RTSPServer::RTSPClientSession::handleRequest(char* request) {
  strcpy((char*)fRequestBuffer, request);
  envir().taskScheduler()
      .rescheduleDelayedTask(fHandleRequestTask,
			     0,
			     (TaskFunc*)handleRequestTask, this);
  return true;
}

void RTSPServer::RTSPClientSession::closeClientSession() {
  fObserver = NULL;
  envir().taskScheduler()
      .rescheduleDelayedTask(fCloseClientSessionTask,
			     0,
			     (TaskFunc*)closeClientSessionTask, this);
}

void RTSPServer::RTSPClientSession::handleRequestTask(RTSPClientSession* clientSession) {
  clientSession->handleRequest();
}

void RTSPServer::RTSPClientSession::closeClientSessionTask(RTSPClientSession* clientSession) {
