...
getMimeType (const char *extension)
{
  ...
  if (!extension)
    return NULL;
#ifdef WIN32
  list = (mime_type_t *)MIME_Type_List;
#else
  list = (struct mime_type_t *)MIME_Type_List;
#endif
                        ...
                        struct upnp_entry_t *entry, const char *container)
{
  ...
  for (i = 0; i < n; i++)
  {
    ...
    sprintf (fullpath, "%s"SLASH_STR"%s", container, namelist[i]->d_name);
...
build_metadata_list (struct ushare_t *ut)
{
  ...
  for (i=0 ; i < ut->contentlist->count ; i++)
  {
    ...
    if (ut->contentlist->content[i][size - 1] == SLASH_CHAR)
      ut->contentlist->content[i][size - 1] = '\0';
    title = strrchr (ut->contentlist->content[i], SLASH_CHAR);
    if (title)
      title++;
    else
    {
